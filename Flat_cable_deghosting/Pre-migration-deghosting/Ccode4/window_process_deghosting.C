...
//  cout << "call blocking!!!" << endl;
  ...
  for (i_iter = 0; i_iter < nw-1; i_iter++) {
...
void window_process_deghosting::inversed_blocking() {
  ...
  for (int i_iter = 0; i_iter < nw; i_iter++) {
    ...
    int lidx = array_of_blocks[i_iter].get_lidx();
    int length = lidx - fidx + 1;
    int igflag = array_of_blocks[i_iter].igflag;
    int qualified = array_of_blocks[i_iter].qualified;
    array_of_blocks[i_iter].get_output_trace(tmpfloat,length);
    
    if (igflag) {
      for (int j_iter = 0; j_iter < length; j_iter++) {
        weight[fidx + j_iter] += 0.000000001;
        trace_out[fidx + j_iter] += 0.000000001*tmpfloat[j_iter];
      }
    } else {
      if (qualified) {
        for (int j_iter = 0; j_iter < length; j_iter++) {
          weight[fidx + j_iter] += 1.0;
          trace_out[fidx + j_iter] += 1.0*tmpfloat[j_iter];
        }
      } else {
        for (int j_iter = 0; j_iter < length; j_iter++) {
          weight[fidx + j_iter] += 0.000000001;
  ...
  for (int j_iter = 0; j_iter < nt; j_iter++) {
    trace_out[j_iter] /= weight[j_iter];
  }
...
//  cout << "Begin to call engine.process!!!" << endl;
  ...
  for (int i_iter = 0; i_iter < nw; i_iter++) {
    check_data_block(&array_of_blocks[i_iter]);
  }
  ...
  for (int i_iter = 0; i_iter < nw; i_iter++) {
    int igflag = array_of_blocks[i_iter].igflag;
    if (igflag) {
      array_of_blocks[i_iter].keep_input();
    } else {
...
float window_process_deghosting::maxabs(float* vec, int length) {
  
  float maxabsv = abs(vec[0]);
  
  for (int i_iter = 1; i_iter < length; i_iter++) {
    
    if (abs(vec[i_iter])>maxabsv) {
      maxabsv = abs(vec[i_iter]);
    }
...
void window_process_deghosting::check_data_block(ghost_data_block* data_block) {
  
  int length = data_block->length;
  float * tmp_float = new float[length];
  float boundaries_criteria = 0.1;
  float maxabsv;
  
  data_block->get_input_trace(tmp_float, length);
  maxabsv = maxabs(tmp_float, length); 
          
  
  if ((abs(tmp_float[0])<boundaries_criteria*maxabsv)&&(abs(tmp_float[length-1])<boundaries_criteria*maxabsv)) {
    data_block->igflag = 0;
  } else {
    data_block->igflag = 1;
  }
