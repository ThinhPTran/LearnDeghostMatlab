...
void General_deghost_engine::calculate_output(ghost_data * ghostdata) {

  
  if ( (tau > tmaxcheck)||(tau < tmincheck) ) {
    ghostdata->qualified = 0;
    for (int i_iter = 0; i_iter < nt; i_iter++) {
      trace_out[i_iter] = trace_in[i_iter];
    }
  } else {
    ghostdata->qualified = 1;
...
void General_deghost_engine::execute(ghost_data * ghostdata) {
