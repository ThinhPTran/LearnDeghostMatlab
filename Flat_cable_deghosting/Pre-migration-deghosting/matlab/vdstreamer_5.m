...
% figure(1)
% plot(t,primary_sig);F = zeros(nt, 1);
F(floor(nt/2) + 1) = 1.0;
F(index) = -1.0;
...
%% Recover
 ...
 F = zeros(nt, 1);
 F(floor(nt/2) + 1) = 1.0;
 F(floor(nt/2) + ind ) = -1.0;
 
 P = crosscorr2(F,withghost);
 
 withghost1 = convolution2(F,P);
...
%  
 ...
 figure(4)
 plot(twindow,withghost,'blue',twindow,P,'red',twindow,primary_sig,'green',twindow,withghost1,'yellow');
 hleg = legend('input data','deghosted data','primary','withghost1');
