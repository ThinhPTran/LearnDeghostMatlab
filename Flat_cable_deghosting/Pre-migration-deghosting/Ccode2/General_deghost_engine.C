...
void General_deghost_engine::calculate_Fourier_image() {
  ...
  fftw_complex * tmp_in = NULL;
  fftw_complex * tmp_out = NULL;
...
void General_deghost_engine::calculate_check_array() {
  ...
  fftw_complex * tmp_in = (fftw_complex *) fftw_malloc(nt*sizeof(fftw_complex));
  fftw_complex * tmp_out = (fftw_complex *) fftw_malloc(nt*sizeof(fftw_complex));
  
  
  for (int i_iter = findxcheck; i_iter < ntcheck; i_iter++) {
    ...
    // Keep it symmetry 
    for (int j_iter = nt-1; j_iter > nt/2; j_iter--) {
...
void General_deghost_engine::calculate_raw_output() {
  ...
  for (int j_iter = 0; j_iter < nt; j_iter++) {
    normweight[j_iter] = abs(P[j_iter] + G[j_iter]);
  }
  maxweight = fmaxf(nt, normweight);
  for (int j_iter = 0; j_iter < nt; j_iter++) {
    normweight[j_iter] /= maxweight;
  }
  
  
  for (int j_iter = 0; j_iter < nt; j_iter++) {
    P[j_iter] = P[j_iter]*normweight[j_iter];
  }
