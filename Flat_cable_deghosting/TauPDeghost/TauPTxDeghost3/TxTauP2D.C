...
double TxTaup2D::absmax(double* input, int lgth) {
  
  double ret = abs(input[0]);
  
  for (int i_iter = 0; i_iter < lgth; i_iter++) {
    double tmp = abs(input[i_iter]);
    if (tmp > ret) {
      ret = tmp;
    }
  }
...
//              out[ip*nt+it-id]+=fabs(dx)*
              out[ip*nt+it-id]+=
                  (in[ix*nt+it]+
                   dfrac*(in[ix*nt+it+1]-in[ix*nt+it]))/nx;
      }
    }
  }
...
//             out[ix*nt+it-id]+=fabs(dp)*
             out[ix*nt+it-id]+=
                  (in[ip*nt+it]+
                   dfrac*(in[ip*nt+it+1]-in[ip*nt+it]))/np;
      }
    }
  }
...
void TxTaup2D::rescale_output() {
  
  int M = nx*nt;
  double maxval1 = *maxval;
  double maxval2 = absmax(out,M);
  double ratio = maxval1/maxval2;
