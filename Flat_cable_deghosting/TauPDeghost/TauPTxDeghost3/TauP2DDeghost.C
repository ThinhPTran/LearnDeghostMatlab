...
void TauP2DDeghost::get_input_info(TauP2DDeghost_info_t info) {
  ...
  mfapp = info.mfapp;
  ifscale = info.ifscale;
  ifaddback = info.ifaddback;
  ifreshape = info.ifreshape;
...
void TauP2DDeghost::display_info_for_checking() {
  ...
  cout << "fmax: " << fmax << endl;
  cout << "nfb: " << nfb << endl;
  cout << "mfapp: " << mfapp << endl;
  cout << "ifscale: " << ifscale << endl;
  cout << "ifaddback: " << ifaddback << endl;
  cout << "ifreshape: " << ifreshape << endl;
...
void TauP2DDeghost::uniform_fftwf2d(int nt, int nx, complex<double>* in, complex<double>* out) {
  ...
  for (int i_iter = 0; i_iter < nt; i_iter++) {
    ...
    uniform_fftw1d(nx, tmp_in, tmp_out, FFTW_FORWARD);
...
void TauP2DDeghost::uniform_ifftwf2d(int nt, int nx, complex<double>* in, complex<double>* out) {
  ...
  for (int i_iter = 0; i_iter < nt; i_iter++) {
    ...
    uniform_fftw1d(nx, tmp_in, tmp_out, FFTW_BACKWARD);
...
//#pragma omp parallel for
  for (int i_iter = 0; i_iter < np; i_iter++) {
    ...
    if (test > 0) {
      ...
      uniform_fftw1d(exnt, tmp_in, tmp_out, FFTW_FORWARD);
...
//#pragma omp parallel for
  for (int i_iter = 0; i_iter < np; i_iter++) {
    ...
    if (test > 0) {
      ...
      uniform_fftw1d(exnt, tmp_in, tmp_out, FFTW_BACKWARD);
...
//#pragma omp parallel for
  ...
  for (int i_iter = 0; i_iter < nfb; i_iter++) {
    if ((freq_hz[i_iter]<mfapp)&&(ratio[i_iter]<1.0)) {
      ratio[i_iter]=1.0;
    }
  }
