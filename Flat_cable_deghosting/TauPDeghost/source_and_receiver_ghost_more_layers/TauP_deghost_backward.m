...
for i_iter = 2:floor(nf/2)+1
  ...
  exfoutput(nf-i_iter+2,:) = conj(exfoutput(i_iter,:));
end
...
%% TauP transform
A = zeros(exnp,exnx);
exoutput1_taup = zeros(exnt,exnp);
exfoutput1_taup = zeros(exnf,exnp);
tmp_in = zeros(exnx,1);
...
for i_iter = 1:floor(nf/2)
  ...
  tmp_in = transpose(exfoutput1(i_iter,:));
  ...
  exfoutput1_taup(i_iter,:) = transpose(tmp_out);
  
end


for i_iter = 2:floor(nf/2)
  exfoutput1_taup(nf-i_iter+2,:) = conj(exfoutput1_taup(i_iter,:)); 
end


exoutput1_taup = real(ifft(exfoutput1_taup));

toc


fid = fopen('exreceiver_data_with_ghost_n1_500_invtaup_fwdtaup_after_filtering_deghost.bin','wb');
fwrite(fid,exoutput1_taup,'single');
fclose(fid);
