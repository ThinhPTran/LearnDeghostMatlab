...
input=input-circshift(input,[0 4]); 
...
%% Deghost
nominator1=1-exp(1i*omega*tausrc); 
nominator2=1+exp(1i*omega*tausrc); 
nominator3=1-exp(1i*omega*taurec); 
nominator4=1+exp(1i*omega*taurec); 

foutput1=finput.*nominator1;
foutput2=finput.*nominator2; 
foutput3=finput.*nominator3; 
foutput4=finput.*nominator4; 
...
output5=(output1+output3)/2.0;
output6=(output2+output4)/2.0; 


for i_iter = 1:nt
  if ((output5(i_iter)*output6(i_iter))<0)
    output7(i_iter) = output5(i_iter) + output6(i_iter);
  else
    output7(i_iter) = output5(i_iter) - output6(i_iter); 
  end
end
...
figure();
plot(t,input_orig,t,input,t,output5,t,output6,t,output7);
legend('orig','input','output5','output6','output7');
...
dbabsfoutput4=mag2db(absfoutput4); 


figure();
plot(1:nf,absfinput,1:nf,absfoutput1,1:nf,absfoutput2,1:nf,absfoutput3,1:nf,absfoutput4); 
legend('absfinput','absfoutput1','absfoutput2','absfoutput3','absfoutput4'); 

figure(); 
plot(1:nf,dbabsfinput,1:nf,dbabsfoutput1,1:nf,dbabsfoutput2,1:nf,dbabsfoutput3,1:nf,dbabsfoutput4); 
legend('dbabsfinput','dbabsfoutput1','dbabsfoutput2','dbabsfoutput3','dbabsfoutput4'); 
