...
%% Estimate spec
nod=30; 
nominator1=(1-exp(1i*omega*tau))/2.0; 
...
for i_iter = 1:nt
  if((abs(output1(i_iter))>2.0*abs(input(i_iter)))||(abs(output1(i_iter))<0.5*abs(input(i_iter))))
    output1(i_iter)=0.0; 
  end
end
...
figure();
plot(1:nf,absfinput,1:nf,absfoutput1); 
legend('absfinput','absfoutput1'); 

figure(); 
plot(1:nf,dbabsfinput,1:nf,dbabsfoutput1); 
legend('dbabsfinput','dbabsfoutput1'); 


figure(); 
plot((1:nf)*df,abs(nominator1)); 
legend('filter'); 
