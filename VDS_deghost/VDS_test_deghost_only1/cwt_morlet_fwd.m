function [output, freq_hz] = cwt_morlet_fwd(x,wmode,Nt,dt,nfb,fmin_hz,fmax_hz,Resolution)
...
for i_iter=1:nfb
    for j_iter=1:Nh+1
        ...
        wavelet(j_iter,i_iter)=exp(-0.5*rarg^2);%*sqrt(pi/log(2))/freq_hz(i_iter);
...
for i_iter=1:nfb
    ...
    time_spectral(:,i_iter)=ifft(out_wavelet);
end
